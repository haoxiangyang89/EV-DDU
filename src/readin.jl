...
# read in the demand correlation data
function readSigmaD(fileNameSigma, fileType = "csv")
    if fileType == "csv"
        # csv file format:
        dataRawSigma = readdlm(fileNameSigma, ',');
        mΣ,nΣ = size(dataRawSigma);
        if mΣ == nΣ
            Σd = dataRawSigma[2:mΣ,2:nΣ];
            ΣTitle = dataRawSigma[1,2:nΣ];
        else
            error("Wrong dimension for the correlation matrix.");
        end
    else
        error("Currently your file type is not supported");
    end
    return Σd,ΣTitle;
end

# generate random demand scenarios with the correlation matrix
function demandGen(dData, scenNo, Σd, ΣTitle, ρ = 0.2)
    # batch generate the demand csv files according to certain distribution
    # fileNameP and fileNameQ provides the mean demand
    dataScen = [];
    Σd = Array{Float64,2}(Σd);
    for ω in 1:scenNo
        # for each scenario, generate demand for each time period
        pd = Dict();
        qd = Dict();
        pdPrint = zeros(length(ΣTitle),dData.T);
        qdPrint = zeros(length(ΣTitle),dData.T);
        for t in 1:dData.T
            # set up the multi-variable normal distribution
            pdmean = Array{Float64}([]);
            qdmean = Array{Float64}([]);
            for i in eachindex(ΣTitle)
                push!(pdmean,ρ*(dData.pd[ΣTitle[i]][t]+1e-8));
                push!(qdmean,ρ*(dData.qd[ΣTitle[i]][t]+1e-8));
            end
            pdDis = MvNormal([dData.pd[i][t] for i in ΣTitle], Array(Hermitian(diagm(pdmean)*Σd*diagm(pdmean))));
            qdDis = MvNormal([dData.qd[i][t] for i in ΣTitle], Array(Hermitian(diagm(qdmean)*Σd*diagm(qdmean))));

            pdGen = max.(rand(pdDis),0);
            for i in eachindex(pdGen)
                if abs(pdGen[i]) < 1e-8
                    pdGen[i] = 0;
                end
            end
            qdGen = rand(qdDis);
            for i in eachindex(qdGen)
                if abs(qdGen[i]) < 1e-8
                    qdGen[i] = 0;
                end
            end
        ...
        for i in eachindex(ΣTitle)
            pd[ΣTitle[i]] = pdPrint[i,:];
            qd[ΣTitle[i]] = qdPrint[i,:];
        end
        push!(dataScen,demandData(dData.T,pd,qd));
    end
    return dataScen;
end

# read in the deterministic RES value
function readRES(fileNameWind, fileNameSolar, fileNameLoc)
    # read in the scenario data
    RESData = Dict();
    RESData["wind"] = readdlm(fileNameWind, ',');
    RESData["solar"] = readdlm(fileNameSolar, ',');
    # obtain the scaling factor for the wind and solar data
    RESData["windScale"] = mean(maximum(RESData["wind"],dims=2));
    RESData["solarScale"] = mean(maximum(RESData["solar"],dims=2));
    if size(RESData["wind"])[2] == size(RESData["solar"])[2]
        RESData["T"] = size(RESData["wind"])[2];
        # obtain the location/type information, nominal value
        locInfo = readdlm(fileNameLoc, ',');
        RESData["type"] = Dict();
        RESData["nominal"] = Dict();
        RESData["locations"] = [];
        mLoc,nLoc = size(locInfo);
        if nLoc == 3
            for i in 1:mLoc
                push!(RESData["locations"], locInfo[i,1]);
                RESData["type"][locInfo[i,1]] = locInfo[i,2];
                RESData["nominal"][locInfo[i,1]] = locInfo[i,3];
            end
        end
        return RESData;
    else
        error("Mismatch in RES generation data dimensions.");
    end
end

# generate random RES scenarios
function RESGen(RESData, scenNo)
    dataScen = [];
    for ω in 1:scenNo
        pg = Dict();
        # for each scenario, generate RES generation for each time period
        for i in RESData["locations"]
            if RESData["type"][i] == "W"
                # wind generations
                scenRandom = rand(1:size(RESData["wind"])[1]);
                pg[i] = RESData["wind"][scenRandom,:]/RESData["windScale"]*RESData["nominal"][i];
            elseif RESData["type"][i] == "S"
                # solar generations
                scenRandom = rand(1:size(RESData["solar"])[1]);
                pg[i] = RESData["solar"][scenRandom,:]/RESData["solarScale"]*RESData["nominal"][i];
            end
        end
        push!(dataScen,genRES(RESData["T"],pg));
    end
    return dataScen;
end

# read in the network, deterministic demand, deterministic RES, battery data
function readInData(i,caseList,T,cz = 1e4, Δt = 0.25)
    fileAdd = "./data/$(caseList[i])/case$(caseList[i])_ieee.m";
    ...
    sigmaAdd = "./data/$(caseList[i])/testDataSigmaD_$(caseList[i]).csv";
    sigmaData = readSigmaD(sigmaAdd,"csv");
    windAdd = "./data/$(caseList[i])/RES_Gen/windScen.csv"
    solarAdd = "./data/$(caseList[i])/RES_Gen/solarScen.csv"
    locAdd = "./data/$(caseList[i])/RES_Gen/locGen.csv"
    gData = readRES(windAdd,solarAdd,locAdd);
    return fData, bData, dData, sigmaData, gData;
end

function readDisruptions(i,caseList,suffix = "",τ = nothing)
    disAddt = "./data/$(caseList[i])/testProbReadt_$(caseList[i]).csv";
    disAddo = "./data/$(caseList[i])/testProbReado_$(caseList[i]).csv";
    pDistr = readDisruption(disAddt,disAddo,"csv",τ);
    return pDistr;
end

function readP2P(i,caseList,scenNo,T)
    genScen = RESGen(gData,scenNo);
    Σd,ΣTitle = readSigmaD("./data/$(caseList[i])/testDataSigmaD_$(caseList[i]).csv");
    demandScen = demandGen(dData, scenNo, Σd, ΣTitle);

    # get the first time period aligned
    pScen = Dict();
    for g in keys(genScen[1].pg)
        avgGen = mean([genScen[i].pg[g][1] for i in 1:scenNo]);
        for i in 1:scenNo
            genScen[i].pg[g][1] = avgGen;
        end
    end
    ...
    Ωt = 1:scenNo;
    rDistr = p2pDistrn(pScen,genScen,demandScen);
    distDict = calcDistance(rDistr,Ωt,T);
    return Ωt, rDistr, distDict;
end

function parallel_load(pDistr_in, Ωt_in, rDistr_in, distDict_in)
    global pDistr = pDistr_in;
    global Ωt = Ωt_in
    global rDistr = rDistr_in
    global distDict = distDict_in
end